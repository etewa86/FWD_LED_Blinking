...
gGPIOError_t gpioPinInterruptConfig(uint32 base,uint8 pin,gInterrSenseControl_t obj){
	gGPIOError_t state = PASS;
	...
	HW_WRITE_BIT(base+GPIO_INTERRUPT_MASK_OFFSET,pin,HIGH);
     ...
     }else if(obj  == FALING_EDGE || obj == RAISNG_EDGE ||
    		 obj == BOTH_EDGES){
...
gGPIOError_t gpioGetRawInterruptStatus(uint32 base , uint8 *status){
	gGPIOError_t state = PASS;
	if(base == BASE_A || base == BASE_B|| base == BASE_C||
	   base == BASE_D || base == BASE_E|| base == BASE_F){
		*status = HW_READ_REG_32BIT(base+GPIO_RAW_INTERRUPT_STATUS_OFFSET);
...
gGPIOError_t gpioGetMaskedInterruptStatus(uint32 base , uint8 *status){
	gGPIOError_t state = PASS;
	if(base == BASE_A || base == BASE_B|| base == BASE_C||
	   base == BASE_D || base == BASE_E|| base == BASE_F){
		*status = HW_READ_REG_32BIT(base+GPIO_MASK_INTERRUPT_STATUS_OFFSET);
